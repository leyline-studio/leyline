//! Process version 5 — the fifth rendering contract of the develop
//! pipeline (ADR 0018).
//!
//! This module *is* the definition of `process: 5`: the exact formulas
...
//! (§3.3).
//!
//! Process 5 differs from process 4 in exactly one place: the lens
//! correction step also corrects transverse chromatic aberration (TCA) —
//! the different radial magnification Lensfun's profile reports per
//! channel, causing color fringes near the frame edges. It runs as an
//! **independent second geometric pass**, right after distortion (still
//! before vignetting and any tonal operator, the first step of §3.1's fixed
//! order): each channel of each output pixel is resampled from its own
//! Lensfun-reported source coordinate, from the buffer distortion has
//! already undistorted — not fused into one combined distortion+TCA
//! remap, a deliberate simplification documented in ADR 0018 (the crate's
//! `apply_subpixel_distortion` computes TCA-only offsets; composing them
//! with the distortion transform into a single pass would need manual
//! coordinate algebra this process version doesn't attempt). No TCA
//! calibration at this focal length leaves the image exactly as process 4
//! would render it. Every other operator is copied from process 4
...
/// already validated and confirmed to declare `process: 5`. `shot` is the
...
mod tests {
    ...
    #[test]
    fn disabled_lens_correction_matches_process_4_bit_for_bit() {
        ...
        let with_process_5 = develop(&image, &settings, Some(&canon_shot(20.0))).unwrap();
        let with_process_4 =
            crate::process4::develop_scaled(&image, &settings, Some(&canon_shot(20.0)), 1.0)
                .unwrap();
        assert_eq!(with_process_5, with_process_4);
    }
    ...
    #[test]
    fn tca_further_changes_the_process_4_render() {
        let image = test_image(640, 480);
        let out5 = develop(&image, &enabled_settings(), Some(&canon_shot(20.0))).unwrap();
        let out4 = crate::process4::develop_scaled(
            &image,
            &enabled_settings(),
        ...
        assert_ne!(
            out5.data, out4.data,
            "TCA correction should change more than distortion + vignetting alone"
        );
    }

    #[test]
    fn no_tca_calibration_matches_process_4_bit_for_bit() {
        // Real bundled entry with distortion data but no TCA data at all
        // (lensfun crate's data/db/slr-canon.xml): distortion still
        // applies, but the TCA pass is a genuine no-op, so process 5
        // renders exactly like process 4.
        let image = test_image(640, 480);
        let shot = LensShot {
            camera_make: "Canon".to_owned(),
            camera_model: "Canon EOS 5D Mark III".to_owned(),
        ...
        let out4 =
            crate::process4::develop_scaled(&image, &enabled_settings(), Some(&shot), 1.0).unwrap();
        assert_eq!(out5, out4);
    }
