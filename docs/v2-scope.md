...
| Pipeline (§3.1) | Un étage **Réglages locaux** s'insère dans le bloc tonal/couleur. Son insertion est déjà un événement de process. **Point dur : le référentiel de coordonnées.** Les masques se dessinent sur l'image **affichée** (après rotation **et** recadrage), or §3.1 applique Rotation/Recadrage **en fin** de chaîne. Il faut un contrat de coordonnées explicite (voir §8). |
| Crate | Géométrie et valeurs des masques : types dans `leyline-core` (`Settings`). Rastérisation masque→couverture : soit dans `leyline-engine`, soit un nouveau crate `leyline-mask` consommé par l'engine, parallèle à `leyline-lens`. La rastérisation paramétrique (radial, gradué, linéaire) est petite ; la brosse l'est moins. |
| Catalogue | Masques **paramétriques** (radial/gradué/linéaire) : compacts, ils tiennent dans `settings_json` — cohérent avec « une révision = état complet et autonome » (`docs/catalog.md` §17). Masques **brosse** : à stocker en **traits vectoriels** (liste d'empreintes x/y/rayon/flux/dureté), jamais en raster, pour rester reproductible et portable. Si une pile de traits devient volumineuse, une table/blob dédiée est une alternative — au prix de casser l'autonomie de `settings_json` (à trancher). |
| API moteur | `EditSession` (`docs/engine-api.md` §10.1) gagne des `Param` de création/sélection/édition de masque. Un nouveau `SettingsGroup` (presets, §10.3) pour les réglages locaux. |
| Complexité | **XL** — c'est de l'infrastructure, pas une fonctionnalité isolée. |
...
2. **Stockage brosse** : traits vectoriels dans `settings_json` vs table dédiée — arbitrage autonomie/volume.
3. **Interaction undo/redo et coalescence** (§17 catalogue) : un trait de brosse est-il une intention (une révision) ou un geste continu à coalescer comme un drag de curseur ?
...
* **Primitive de masquage générique.** L'item 2 est l'infrastructure sur laquelle s'adossent l'item 5 (spatial), le dehaze masqué (6) et le color grading régional (3). Construire le masquage d'abord évite de recoder trois fois la même couverture spatiale. Le color grading (3), la courbe (4) et clarté/texture/dehaze (6) restent livrables **en global** sans attendre les masques ; leur version régionale, elle, en dépend.
...
| Item | ADR propre ? |
|---|---|
| 2 — Réglages locaux / masqués | Oui — infrastructure + nouveau process + choix de référentiel et de stockage. Probablement plusieurs ADR (primitive de masque, puis chaque type d'outil). |
