//! Process version 7 — the seventh rendering contract of the develop
//! pipeline (ADR 0032).
//!
//! This module *is* the definition of `process: 7`: the exact formulas
...
//! (§3.3).
//!
//! Process 7 differs from process 6 in exactly one place: a spot removal
//! stage runs immediately after lens correction and before white balance —
//! the earliest tonal-adjacent point in the pipeline, so cloning samples data
//! as close to linear as possible. Each `spot_removal[]` entry is a
//! deterministic bilinear clone (no *heal*, ADR 0032): the disk around
//! `source` is copied onto the disk around `target`, feathered at the edge
//! and modulated by `opacity`, applied in list order (later entries can
//! sample pixels earlier entries already touched). `target`/`source` are
//! normalized `[0, 1]` in the post-rotation, pre-crop referential of ADR 0026
//! — the same as [`Crop`] — so they are mapped back to this still-unrotated
//! buffer through the exact inverse of the transform [`rotate`] applies
//! later. No spots leaves the image exactly as process 6 would render it.
//! Every other operator is copied from process 6 unchanged, so this module
//! stays self-contained and frozen.
//!
...
use leyline_core::{Crop, CurvePoint, Point, Settings, SpotRemoval, WhiteBalance};
...
/// already validated and confirmed to declare `process: 7`. `shot` is the
...
fn post_rotation_point_to_buffer(
    width: u32,
    height: u32,
    degrees: f64,
    point: Point,
...
mod tests {
    ...
    #[test]
    fn disabled_lens_correction_matches_process_6_bit_for_bit() {
        ...
        let with_process_7 = develop(&image, &settings, Some(&canon_shot(20.0))).unwrap();
        let with_process_6 =
            crate::process6::develop_scaled(&image, &settings, Some(&canon_shot(20.0)), 1.0)
                .unwrap();
        assert_eq!(with_process_7, with_process_6);
    }
    ...
    #[test]
    fn no_points_matches_process_6_bit_for_bit() {
        let image = test_image(64, 48);
        let settings = Settings {
            process: 7,
            exposure: 0.2,
        ...
        let out7 = develop(&image, &settings, None).unwrap();
        let out6 = crate::process6::develop_scaled(&image, &settings, None, 1.0).unwrap();
        assert_eq!(out7, out6);
    }
    ...
    #[test]
    fn no_spots_matches_process_6_bit_for_bit() {
