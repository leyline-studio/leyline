//! Process version 6 — the sixth rendering contract of the develop
//! pipeline (ADR 0030).
//!
//! This module *is* the definition of `process: 6`: the exact formulas
...
//! (§3.3).
//!
//! Process 6 differs from process 5 in exactly one place: a tone curve
//! operator runs after Whites/Blacks and before Vibrance/Saturation —
//! `tone_curve.points`, interpolated with a monotone cubic Hermite spline
//! (Fritsch–Carlson) and precomputed into a lookup table (same convention as
//! the transfer tables below), applied identically to every channel of the
//! gamma-encoded working buffer (luminance, not per-channel R/G/B — ADR
//! 0030). No points leaves the image exactly as process 5 would render it.
//! Every other operator is copied from process 5 unchanged, so this module
...
use leyline_core::{Crop, CurvePoint, Settings, WhiteBalance};
...
mod tests {
    ...
    #[test]
    fn disabled_lens_correction_matches_process_5_bit_for_bit() {
    ...
    #[test]
    fn no_points_matches_process_5_bit_for_bit() {
        let image = test_image(64, 48);
        let settings = Settings {
            process: 6,
            exposure: 0.2,
