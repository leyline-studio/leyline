...
| [0077](0077-application-updates.md) | Se mettre à jour : dernier manque de la distribution d'[ADR 0019](0019-distribution-i18n.md). Manifeste JSON **signé** (`cargo-packager-updater`, clé privée hors dépôt et hors CI), publié en asset de release et interrogé à la redirection stable `releases/latest/download/latest.json` — **compilée en dur**, parce qu'une URL de mise à jour réglable est une surface de détournement. GitHub Releases plutôt qu'un serveur du projet : un binaire installé interroge son URL pendant des années, et un projet local-first n'opère pas un service dont chaque installation dépend. Vérification **manuelle seule** (Aide ▸ Rechercher des mises à jour…) : un clic *est* le consentement que `vision.md` exige, là où toute vérification périodique demanderait d'abord l'écran qui pose la question — déféré à l'ADR du panneau Préférences, où le choix de langue d'ADR 0019 attend déjà. Rien n'est transmis, hors ligne n'est pas une erreur, trouver une version ne l'installe pas. Et, indépendamment du réseau : `catalog.db` est **copié dans `Backups/` avant toute migration** — elles sont sans retour arrière et `Catalog::open` refuse un catalogue plus récent, donc une mise à jour à un clic rend « revenir en arrière » atteignable par accident ; échec de copie = échec d'ouverture |
