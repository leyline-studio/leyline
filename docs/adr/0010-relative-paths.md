...
* The limit this draws, and it is a real one: **one library covers one tree**. A catalog cannot span two volumes, and a collection already on disk is catalogued in place only by choosing a root above it. Lightroom does span volumes — it anchors each photo to a *named volume* plus a path, and shows the photos of an unplugged disk as offline — which is exactly the absolute path rejected here, with exactly the fragility this ADR is about. Lifting the limit without giving that up is [ADR 0085](0085-named-roots.md)'s decision — named roots, identified by a marker file and located by an advisory hint outside the catalog — not an adjustment of this one: nothing above changes, and a library with a single root is still exactly the library described here.
