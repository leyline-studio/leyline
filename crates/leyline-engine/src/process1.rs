//! Process version 1 — the first rendering contract of the develop pipeline.
//!
//! This module *is* the definition of `process: 1`: the exact formulas below,
//! applied in the fixed order of `docs/pipeline.md` §3.1, are frozen. Any
//! change that alters the pixels they produce must land as a new process
//! version in a new module — this one is kept as-is forever (§3.3).
//!
...
//!   light internally.
//!
//! Lens correction is declared in schema 1 but **not rendered** by process 1:
//! profile-based corrections arrive with a later process version. This is
//! part of the contract — `enabled: true` renders identically to `false`.
...
///
/// Tanner Helland's polynomial fit, frozen as part of process 1. Channels
